    ...
    module Jobs
      class ScheduledOrphanedDiskCleanup < BaseJob
        ...
        def self.has_work(params)
          ...
          Models::Network.where(Sequel.lit('orphaned = true and created_at < ?', time)).any?
        end
        ...
        def perform
          ...
          old_orphans = Models::Network.where(Sequel.lit('orphaned = true and created_at < ?', time))
          ...
          old_orphans.each do |old_orphan|
            stage.advance_and_track("#{old_orphan.name}") do
              begin
                @orphan_network_manager.delete_network(old_orphan)
              rescue Exception => e
          ...
          raise Bosh::Clouds::CloudError.new(output) if failed_orphan_network_count > 0
          output
        end
      end
    end
